	...
	public static void main(String[] args) {
		...
		while(check) {
			...
			if (select==1){
				...
				int [] studentKorscp = new int [studentInfocp.length];//국어
				int [] studentEglscp = new int [studentInfocp.length];//영어
				int [] studentMathcp = new int [studentInfocp.length];//수학
				int [] totalscp = new int [studentInfocp.length];//총점
				int [] totalccp = new int [studentInfocp.length];
				
				for(int i=0; i<totalStudent.length; i++) {
						studentInfocp[i] = studentInfo[i];
						studentCountcp[i] = studentCount[i];
						studentKorscp[i] = studentKorscp[i];
						studentEglscp[i] = studentEgls[i];
						studentMathcp[i] = studentMath[i];
						totalscp[i] = totals[i];
						totalccp[i] = totalc[i];
				...
				studentInfocp[totalStudent.length] = sc.next();
				System.out.println("추가할 학생 번호 입력");
				studentCountcp[totalStudent.length] = sc.nextInt();
				System.out.println("추가할 학생 국어점수 입력");
				studentKorscp[totalStudent.length] = sc.nextInt();
				System.out.println("추가할 학생 영어점수 입력");
				studentEglscp[totalStudent.length] = sc.nextInt();
				System.out.println("추가할 학생 수학점수 입력");
				studentMathcp[totalStudent.length] = sc.nextInt();
				totalscp[totalStudent.length] = studentKorscp[totalStudent.length]+studentEglscp[totalStudent.length]+studentMathcp[totalStudent.length];
				totalccp[totalStudent.length] = totalscp[totalStudent.length]/3;
